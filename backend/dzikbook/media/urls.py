from django.urls import path
from .views import (
    PhotoManagementView,
    VideoManagementView,
    ProfilePhotoView,
    SigInUserProfilePhotoView
)
# TODO: zmieniłem media_id na photo_id
# TODO: zamiast set profile photo mamy typ metody post
urlpatterns = [
    path('photo/', PhotoManagementView.as_view(), name='upload_photo'),
    path('photo/<int:photo_id>/', PhotoManagementView.as_view(), name='photo_management'),
    path('video/', VideoManagementView.as_view(), name='upload_video'),
    path('video/<int:video_id>/', VideoManagementView.as_view(), name='video_management'),
    path('profile/', SigInUserProfilePhotoView.as_view(), name='sig_in_user_profile_photo'),
    path('profile/<int:user_id>/', ProfilePhotoView.as_view(), name='profile_photo')
]


